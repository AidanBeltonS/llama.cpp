...
void ggml_cuda_mul_mat_q(
    ...
    if (!ids) {
        ...
        // Reuse the previous quantization if this matmul has the same src1 and scale layout.
        auto & cache = ctx.mmq_src1;
        const int ds_layout = use_native_fp4 ? -2 : (int) mmq_get_q8_1_ds_layout(src0->type);

        bool use_cache = !use_native_fp4;
        if (use_cache && cache.ptr == nullptr) {
            cache.pool = &ctx.pool();
            cache.ptr  = (char *) cache.pool->alloc(nbytes_src1_q8_1, &cache.size);
        }
        use_cache = use_cache && cache.pool == &ctx.pool() && cache.size >= nbytes_src1_q8_1;

        const bool cache_hit = use_cache && cache.src1 == src1 && cache.data == src1->data &&
            cache.layout == ds_layout && cache.nbytes >= nbytes_src1_q8_1;
