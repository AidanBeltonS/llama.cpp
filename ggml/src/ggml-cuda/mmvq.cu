...
static __global__ void mul_mat_vec_q(
    ...
    static_assert(has_fusion || !has_gate, "has_gate requires has_fusion");
    ...
    if constexpr (has_fusion) {
        ...
        use_gate_bias = fusion.gate_bias != nullptr && has_gate;
        vgate         = fusion.gate;
        ...
        if constexpr (type == GGML_TYPE_NVFP4) {
            ...
            use_gate_scale = fusion.gate_scale != nullptr && has_gate;
...
#pragma unroll
    ...
    [[maybe_unused]] __shared__ float tmp_shared_gate[(has_gate && (nwarps-1 > 0)) ? nwarps-1 : 1][ncols_dst][rows_per_cuda_block][warp_size];
...
#pragma unroll
    ...
    if constexpr (!has_fusion) {
        GGML_UNUSED_VARS(use_bias, use_gate_bias, use_scale, use_gate_scale, active_glu, glu_limit, gate_bias, x_bias, x_scale, gate_scale, tmp_gate);
    }
    if constexpr (!has_gate) {
        GGML_UNUSED_VARS(use_gate_bias, use_gate_scale, active_glu, glu_limit, gate_bias, gate_scale, gate_biases, gate_scales, tmp_gate);
    }
    if constexpr (type != GGML_TYPE_NVFP4) {
...
template <ggml_type type, int c_rows_per_block, bool has_fusion = false, bool has_gate = false>
...
#pragma unroll
    ...
    if constexpr (!has_fusion) {
        GGML_UNUSED_VARS(tmp_gate, vgate, x_bias, gate_bias, active_glu, glu_limit, x_scale, gate_scale);
    } else if constexpr (type != GGML_TYPE_NVFP4) {
        GGML_UNUSED_VARS(x_scale, gate_scale);
    }
    if constexpr (!has_gate) {
        GGML_UNUSED_VARS(tmp_gate, vgate, gate_bias, active_glu, glu_limit, gate_scale);
    }
}
...
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 1) {
        if (has_fusion) {
            if (fusion.gate != nullptr) {
                launch(mul_mat_vec_q<type, c_ncols_dst, true, true, small_k, halve_iters>);
            } else {
                launch(mul_mat_vec_q<type, c_ncols_dst, true, false, small_k, halve_iters>);
            }
            return;
        }
    }
    ...
    launch(mul_mat_vec_q<type, c_ncols_dst, false, false, small_k, halve_iters>);
}
...
static void mul_mat_vec_q_moe_launch(
    ...
    if (fusion.gate != nullptr) {
        launch(mul_mat_vec_q_moe<type, rows_per_block, true, true>);
    } else if (has_fusion) {
        launch(mul_mat_vec_q_moe<type, rows_per_block, true, false>);
    } else {
        launch(mul_mat_vec_q_moe<type, rows_per_block, false, false>);
    }
}
...
void ggml_cuda_mul_mat_vec_q(
    ...
    // Reuse the previous quantization if this matmul has the same src1. Unlike the MMQ path
    // this does not depend on src0->type at all, quantize_row_q8_1_cuda ignores it.
    auto & cache = ctx.mmvq_src1;
    if (cache.ptr == nullptr) {
        cache.pool = &ctx.pool();
        cache.ptr  = (char *) cache.pool->alloc(nbytes_src1_q8_1, &cache.size);
    }
    const bool use_cache = cache.pool == &ctx.pool() && cache.size >= nbytes_src1_q8_1;
    const bool cache_hit = use_cache && cache.src1 == src1 && cache.data == src1->data &&
        cache.nbytes >= nbytes_src1_q8_1;

    ggml_cuda_pool_alloc<char> src1_q8_1_own(ctx.pool());
    char * src1_q8_1_ptr = use_cache ? cache.ptr : src1_q8_1_own.alloc(ctx.pool(), nbytes_src1_q8_1);
