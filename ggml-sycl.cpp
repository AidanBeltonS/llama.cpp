...
static void dequantize_block_q4_K(const void * __restrict__ vx, dst_t * __restrict__ yy, sycl::vec<uint8_t, 4>* scales_vec_local,
    ...
    const int i = tidx/32;
    const int sg_group = item_ct1.get_local_linear_id()/32;
    ...
    sycl::vec<dst_t, 2>* y_vec = reinterpret_cast<sycl::vec<dst_t, 2>*>(yy + i*QK_K + 64*il + n*ir);

    sycl::vec<dst_t, 2> y_tmp1;
    sycl::vec<dst_t, 2> y_tmp2;
    for (int l = 0; l < 2; ++l) {
        y_tmp1[l] = d1 * (q_vec[l] & 0xF) - m1;
        y_tmp2[l] = d1 * (q_vec[l+2] & 0xF) - m1;
    }
    y_vec[0] = y_tmp1;
    y_vec[1] = y_tmp2;

    for (int l = 0; l < n; ++l) {
        y_tmp1[l] = d2 * (q_vec[l] >>  4) - m2;
        y_tmp2[l] = d2 * (q_vec[l+2] >>  4) - m2;
    }
    y_vec[32/2] = y_tmp1;
    y_vec[32/2 + 1] = y_tmp2;
#else
